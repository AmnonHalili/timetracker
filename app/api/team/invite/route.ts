...
export async function POST(req: Request) {
    ...
    try {
        ...
        // Check if user exists and is a real account (not PENDING)
        if (existingUser && existingUser.status !== "PENDING") {
            // Check if user is already a member (including INVITED/PENDING)
            ...
            if (existingMembership) {
                // If they are already ACTIVE, return error
                ...
                await prisma.projectMember.update({
                    ...
                    data: {
                        status: "INVITED",
                        role: role || "EMPLOYEE",
                        joinedAt: new Date() // Refresh join time
                    }
                })
            } else {
