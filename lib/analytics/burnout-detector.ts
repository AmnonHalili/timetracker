...
/**
 * Checks if work was done during weekend
 */
...
/**
 * Checks if work was done late at night (after 20:00)
 */
...
/**
 * Gets aggregated weekly data for a user
 */
...
/**
 * Gets today's overtime hours (beyond daily target)
 */
...
/**
 * Calculates productivity trend over last N days
 */
async function calculateProductivityTrend(userId: string, days: number = 14): Promise<number> {
    ...
    if (snapshots.length < 7) return 0 // Need at least a week of data
...
/**
 * Generates recommendations based on burnout factors
 */
function generateBurnoutRecommendations(factors: BurnoutFactor[]): string[] {
    ...
    // Add general recommendations if critical
 ...
 * Analyzes multiple indicators and returns comprehensive assessment
 */
export async function detectBurnoutRisk(userId: string): Promise<BurnoutAssessment> {
    ...
    if (productivityTrend < -0.20) { // 20% decline
        factors.push({
        ...
        score += 25
    } else if (productivityTrend < -0.10) { // 10% decline
        factors.push({
            type: 'DECLINING_PERFORMANCE',
            severity: 'MEDIUM',
            message: `Productivity declined by ${Math.abs(productivityTrend * 100).toFixed(0)}%`
...
/**
 * Quick check if user has ANY burnout risk
 */
...
/**
 * Gets burnout status for multiple users (for managers)
 */
