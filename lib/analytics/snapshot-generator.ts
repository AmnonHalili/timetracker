...
import { calculateFocusScore, calculateEfficiencyScore, calculatePeakHours, analyzeWorkPatterns } from "./productivity-calculator"
import { detectBurnoutRisk } from "./burnout-detector"

/**
 * Generates a daily analytics snapshot for a user
...
export async function generateDailySnapshot(userId: string, date: Date = new Date()): Promise<void> {
    ...
    entries.forEach(entry => {
        ...
        // Count productive hours (sessions > 30 min without too many breaks)
        ...
        // Check for late work (after 20:00)
    ...
    // Balance score (simple: penalize late work and overtime)
...
/**
 * Backfills snapshots for a user for the last N days
 * Useful for onboarding or recovering missing data
 */
