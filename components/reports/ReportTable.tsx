    ...
    return (
        <div className="rounded-md border">
            <Table>
                <TableHeader>
                    <TableRow>
                        <TableHead className={cn(isRTL ? "text-right" : "text-left", "w-[15%]")}>{t('reports.date')}</TableHead>
                        <TableHead className={cn(isRTL ? "text-right" : "text-left", "w-[21.25%]")}>{t('reports.day')}</TableHead>
                        <TableHead className={cn(isRTL ? "text-right" : "text-left", "w-[21.25%]")}>{t('reports.startTime')}</TableHead>
                        <TableHead className={cn(isRTL ? "text-right" : "text-left", "w-[21.25%]")}>{t('reports.endTime')}</TableHead>
                        <TableHead className={cn(isRTL ? "text-left" : "text-right", "w-[21.25%]")}>{t('reports.totalHours')}</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    {days.map((day) => (
                        <TableRow key={day.date.toISOString()} className={cn(!day.isWorkDay && "bg-muted/30")}>
                            ...
                            <TableCell className={cn(isRTL ? "text-left" : "text-right", "font-mono", "w-[21.25%]")}>
